...
app.post('/std', function (req, res) {
    ...
    dbConn.query("INSERT INTO student SET ?", std, function (error, results, fielde) {
        if (error) throw error;
        return res.send(results);
    });
});
...
app.delete('/std/:id', function(req,res){
    ...
    dbConn.query('DELETE FROM student WHERE std_id = ?',std_id, function (error, results, fields) {
        if (error) throw error;
        return res.status(400).send({ error: true, message: 'Student has been updated successfully' });
    });
})
